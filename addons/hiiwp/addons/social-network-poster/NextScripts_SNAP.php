...
*/
define( 'NextScripts_SNAP_Version' , '3.6.6' ); 
...
global $nxs_snapAvNts, $nxs_snapThisPageUrl, $nxs_snapSetPgURL, $nxs_plurl, $nxs_plpath, $nxs_isWPMU, $nxs_tpWMPU, $nxs_skipSSLCheck; define( 'NXS_PLPATH', dirname(__FILE__).'/' );  define( 'NXS_PLURL', $link);  define( 'NXS_SETV', 350); 

$nxs_snapSetPgURL = nxs_get_admin_url().'options-general.php?page=NextScripts_SNAP.php'; $nxs_snapThisPageUrl = $nxs_snapSetPgURL; $nxs_plurl = $link; $nxs_plpath = dirname(__FILE__).'/'; 
...
if (!function_exists("NS_SNAutoPoster_ap")) { function NS_SNAutoPoster_ap() { global $plgn_NS_SNAutoPoster, $nxs_plurl; // if (!isset($plgn_NS_SNAutoPoster)) return; $options = $plgn_NS_SNAutoPoster->nxs_options;       
   if (function_exists('add_options_page')) { add_options_page('Social Networks Auto Poster', 
     'Social Networks Auto Poster', 'manage_options', basename(__FILE__), array(&$plgn_NS_SNAutoPoster, 'showSNAutoPosterOptionsPage'));          
}}}
...
if (!function_exists("nxssnap_enqueue_scripts")) { function nxssnap_enqueue_scripts(){ if (function_exists('get_current_screen')) $screen = get_current_screen(); else return; // prr($screen->id);   // prr($screen);      
  if( $screen->id == 'settings_page_NextScripts_SNAP' || $screen->id=='toplevel_page_NextScripts_SNAP-network' || $screen->base == 'post' || $screen->id =='toplevel_page_NextScripts_SNAP-network') {
	  $file = dirname(__FILE__); // Current PHP file, but can be anyone
	  $link = str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, $file );
      wp_enqueue_script( 'nxssnap-scripts', $link . '/js/js.js', array( 'jquery' ),  NextScripts_SNAP_Version);
      wp_enqueue_style( 'nxssnap-style',  $link . '/js/snap.css', array(),  NextScripts_SNAP_Version );
      
      wp_enqueue_script( 'selectize', $link . '/js/selectize.min.js', array( 'jquery' ),  NextScripts_SNAP_Version);
      wp_enqueue_style( 'selectize',  $link . '/js/selectize.css', array( ),  NextScripts_SNAP_Version );
      
      wp_enqueue_script( 'tokenize', $link . '/js/jquery.tokenize.js', array( 'jquery' ),  NextScripts_SNAP_Version);
      wp_enqueue_style( 'tokenize',  $link . '/js/jquery.tokenize.css', array( ),  NextScripts_SNAP_Version );
      ...
      if( $screen->id == 'settings_page_NextScripts_SNAP' ||  $screen->id=='toplevel_page_NextScripts_SNAP-network' ) {  wp_enqueue_script( 'jquery-ui-datepicker' );            
        wp_enqueue_style( 'jquery-ui-datepicker',  $link . '/js/jquery-ui.css', array( ),  NextScripts_SNAP_Version );
      }      
