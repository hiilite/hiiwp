...
if ( ! class_exists( 'GADWP_Backend_Setup' ) ) {
	...
	final class GADWP_Backend_Setup {
		...
		public function load_styles_scripts( $hook ) {
			...
			wp_enqueue_script( 'gadwp-backend-ui', $link.'/js/ui.js', array( 'jquery' ), GADWP_CURRENT_VERSION, true );
			...
			if ( in_array( $new_hook, $settings_hooks ) ) {
				...
				$file = dirname(__FILE__).'/'; // Current PHP file, but can be anyone
				$link = str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, $file );
				wp_enqueue_script( 'wp-color-picker-script-handle', $link .'/js/wp-color-picker-script.js', array( 'wp-color-picker' ), false, true );
				wp_enqueue_script( 'gadwp-settings', $link.'/js/settings.js', array( 'jquery' ), GADWP_CURRENT_VERSION, true );
			}
		}
